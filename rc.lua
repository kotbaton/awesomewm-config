...
-- Default apps and autostart file
...
local function set_wallpaper(s)
    ...
    if beautiful.wallpaper then
        ...
        -- gears.wallpaper.centered(wallpaper, s, "#1d2021", 1)
		-- gears.wallpaper.fit(wallpaper, s, "#1d2021")
		-- gears.wallpaper.tiled(wallpaper, s, 0)
    end
	--]]
end
...
awful.screen.connect_for_each_screen(function(s)
	...
	--s.textclock:buttons(gears.table.join(awful.button({ }, 1, function () awful.spawn("gsimplecal", false) end)))
	s.textclock:buttons(gears.table.join(awful.button({},1, function() 
		if s.textclock.format == "%H:%M" then
			s.textclock.format = "[%d %b, %a] %H:%M"
		else
			s.textclock.format = "%H:%M"
		end
	end)))
...
----------------------{ START APPS }--------------------------------------------
	...
	awful.key({modkey}, "c", function()
		local textclock = mouse.screen.textclock
		if textclock.format == "%H:%M" then
			textclock.format = "[%d %b, %a] %H:%M"
		else
...
----------------------{ AWESOME }--------------------------------------------
	...
	awful.key({ modkey, 		  }, "space",	 awful.screen.focused().keyboardlayout.next_layout,	 {description="change language", group="awesome"}),
...
----------------------{ SOUND }--------------------------------------------
	awful.key({ }, "XF86AudioRaiseVolume",	
		function() 
			-- awful.util.spawn(gears.filesystem.get_configuration_dir() .. "scripts/volnoti.sh u", false)
			widgets.volume.control("increase")
		end, {description="Increase volume", group="Volume"}),
	awful.key({ }, "XF86AudioLowerVolume",	
		function()
			-- awful.util.spawn(gears.filesystem.get_configuration_dir() .. "scripts/volnoti.sh d", false)
			widgets.volume.control("decrease")
		end, {description="Decrease volume", group="Volume"}),
	awful.key({ }, "XF86AudioMute",	
		function()
			-- awful.util.spawn(gears.filesystem.get_configuration_dir() .. "scripts/volnoti.sh m", false)
...
----------------------{ CMUS }--------------------------------------------
	awful.key({ modkey }, "F1",
		...
		end, {description="Toggle Pause", group="cmus"}),
	awful.key({ modkey }, "F2", 
		function() 
			awful.util.spawn("cmus-remote -r", false)
		end, {description="Prev", group="cmus"}),
	awful.key({ modkey }, "F3", 
		function() 
			awful.util.spawn("cmus-remote -n", false)
		end, {description="Next", group="cmus"}),
		--[[
	awful.key({ modkey }, "F1",
		function() 
		...
		end, {description="Toggle Pause", group="Spotify"}),
	awful.key({ modkey }, "F2", 
		function() 
		...
		end, {description="Prev", group="Spotify"}),
	awful.key({ modkey }, "F3", 
		function() 
...
--[[
    awful.key({ "Mod1",           }, "Tab",
        ...
        end, {description = "last focus", group = "client"}),
--]]
...
awful.rules.rules = {
    ...
    {rule_any = { -- Rules for floating clients.
					instance = {"DTA","copyq"}, 
	...
	{ rule = { name = "galculator" }, properties = { floating = true, ontop = true }},
	{ rule = { name = "Media viewer" }, properties = { floating = true, ontop = true, titlebars_enabled = false, fullscreen = true }},
}
...
client.connect_signal("request::titlebars", function(c)
    ...
    awful.titlebar(c, {	size = 16,
						...
						position = "top", 
						bg = beautiful.bg_normal
						--bgimage_focus = beautiful.titlebar_top_bgimage_focus,
						--bgimage = beautiful.titlebar_top_bgimage,
					}):setup{
					{ -- Left
						layout  = wibox.layout.fixed.horizontal,
